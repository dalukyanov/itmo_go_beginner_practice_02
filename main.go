...
func main() {
	if len(os.Args) != 2 {
		...
		fmt.Println("Usage: %s <yaml-file>", os.Args[0])
		os.Exit(255)
	}
...
func validateResourceList(filePath string, node *yaml.Node, fieldName string) []string {
	...
	if cpuNode, exists := fields["cpu"]; exists {
		if cpuNode.Kind != yaml.ScalarNode {
			errors = append(errors, fmt.Sprintf("%s:%d cpu must be int", filePath, cpuNode.Line))
		} else {
			if cpuNode.Tag != "!!int" {
				errors = append(errors, fmt.Sprintf("%s:%d cpu must be int", filePath, cpuNode.Line))
			} else {
	...
	if memoryNode, exists := fields["memory"]; exists {
		if memoryNode.Kind != yaml.ScalarNode || memoryNode.Tag != "!!str" {
			errors = append(errors, fmt.Sprintf("%s:%d memory must be string", filePath, memoryNode.Line))
		} else {
			memory := memoryNode.Value
			if memory == "" {
				errors = append(errors, fmt.Sprintf("%s:%d memory is required", filePath, memoryNode.Line))
			} else if !validMemoryUnit.MatchString(memory) {
				errors = append(errors, fmt.Sprintf("%s:%d memory has invalid format '%s'", filePath, memoryNode.Line, memory))
			}
		}
	}
